...
from ics_demo.dao import mon_dao
...
class BaseHandler(BaseHandler):
    def get(self):
        self.write("Vsan Mon Manage")


class MonHandler(BaseHandler):
    def get(self, mon_id=None):
        """
            Instructions:
            In GET:
        ...
        """
        self.write(json.dumps({'mon': self.get_from_dao(mon_dao, mon_id)}))
    ...
    def post(self):
        ...
        if 'mon' not in self.post_data.keys():
            raise CannotParsedError('dict of Mon', str(self.post_data))
        else:
            post_dict = self.deref_user_data(self.post_data['mon'])
        # do rpc
        warrens = {}
